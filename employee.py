...
class employeeClass:
    def __init__(self,root):
        ...
        self.EmployeeTable=ttk.Treeview(emp_frame,columns=("Emp","Name","Gender","Email","Contact","DOB","DOJ","Password","UserType","Address","Salary"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        scrollx.config(command=self.EmployeeTable.xview)
        scrolly.config(command=self.EmployeeTable.yview)
        self.EmployeeTable.heading("Emp",text="EmpID")
        ...
        self.EmployeeTable.column("Emp",width=90)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                cur.execute("Select * from employee where \"Emp ID\"=?",(self.var_emp_id.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Employee ID already assigned,try different",parent=self.root)
                else:
                    cur.execute("Insert into employee(Emp,Name,Gender,Email,Contact,DOB,DOJ,Password,UserType,Address,Salary)values(?,?,?,?,?,?,?,?,?,?,?)",(
                        self.var_emp_id.get(),
                        self.var_name.get(),
                        self.var_gender.get(),
                        self.var_email.get(),
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row!=None:
                    ...
                    cur.execute("Update employee set Name=?,Gender=?,Email=?,Contact=?,DOB=?,DOJ=?,Password=?,UserType=?,Address=?,Salary=? where Emp=?",(
                        self.var_name.get(),
                        self.var_gender.get(),
                        self.var_email.get(),
                        self.var_contact.get(),
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row!=None:
                    ...
                    if op==True:
                        cur.execute("delete from employee where Emp=?",(self.var_emp_id.get(),))
                        con.commit()
