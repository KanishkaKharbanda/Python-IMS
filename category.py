...
class categoryClass:
    def __init__(self,root):
        ...
        btn_add=Button(self.root,command=self.add,text="Add",font=("goudy old style",15,"bold"),bg="#4caf50",fg="white",cursor="hand2").place(x=360,y=170,width=150,height=30)
        btn_delete=Button(self.root,command=self.delete,text="Delete",font=("goudy old style",15,"bold"),bg="red",fg="white",cursor="hand2").place(x=520,y=170,width=150,height=30)

        #==Category Details==

        cat_frame=Frame(self.root,bd=3,relief=RIDGE)
        cat_frame.place(x=700,y=100,width=390,height=100)
    ...
    def delete(self):
        ...
        try:
            if self.var_cat_id.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","IError, Please Try Again",parent=self.root)
                else:
